...
// another's, and — since OpenRouter was added alongside Ollama — switching
...
describe("organisation-scoped AI provider", () => {
  ...
  it("returns a real OllamaProvider once connected and active by default", async () => {
    ...
    // OpenRouter existing at all, so Ollama being the implicit default is
    // what keeps every organisation that connected before this feature
    // shipped working exactly as it did.
  ...
  describe("OpenRouter", () => {
    ...
    it("both providers stay independently connected regardless of which is active", async () => {
      ...
      // host down, try OpenRouter, host comes back) must never mean
      // re-entering something already entered.
      await setOllamaProvider(organisationId, {
        baseUrl: "https://ollama.example.test",
      });
